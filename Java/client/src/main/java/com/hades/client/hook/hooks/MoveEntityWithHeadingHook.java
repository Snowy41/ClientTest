...
/**
 * Intercepts EntityLivingBase.moveEntityWithHeading(float strafe, float forward)
...
public class MoveEntityWithHeadingHook {
    ...
    public static int[] discretize(double diff) {
        if (Math.abs(diff) <= 22.5)        return new int[]{1, 0};
        if (Math.abs(diff - 45) <= 22.5)   return new int[]{1, -1};
        if (Math.abs(diff + 45) <= 22.5)   return new int[]{1, 1};
        if (Math.abs(diff - 90) <= 22.5)   return new int[]{0, -1};
        if (Math.abs(diff + 90) <= 22.5)   return new int[]{0, 1};
        if (Math.abs(diff - 135) <= 22.5)  return new int[]{-1, -1};
        if (Math.abs(diff + 135) <= 22.5)  return new int[]{-1, 1};
        return new int[]{-1, 0};
    }
    ...
    public static void onEnter(@Advice.This Object entityLivingBase,
        ...
        try {
            auraSideways = false;
            if (entityLivingBase != com.hades.client.api.HadesAPI.player.getRaw()) return;
            ...
            if (rotYawField == null) rotYawField = resolveField(entityLivingBase.getClass(), YAW_NAMES);
